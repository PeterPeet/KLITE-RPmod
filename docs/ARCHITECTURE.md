...
| `characters/builder-rules.js`, `builder.js` | `window.KLITE_RPMod_Builder` | Character builder (levels 1–3) + level up (§5b) |
...
- **SRD data** (`src/data/srd52.js`, generated, 114 KB): `SRD.{attribution, classes, backgrounds,
  ...
  Texts are extracted (page headers and feature tables stripped); core traits, spell
  progression, backgrounds, weapons and armor are transcribed in the script.
- **Builder** (`builder-rules.js` pure, `builder.js` window `builder`): choices object
  ...
  classSkills, speciesSkills, extraSkills, expertise, fightingStyle, languages,
  classEquipment, backgroundEquipment, alignment, name`) → `validate()` + `buildSheet()`;
