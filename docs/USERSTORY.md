...
| Combat distance (R7) | **Zone combat instead of a grid** (2026-09-23, revised 2026-09-25) | Every creature stands in a zone of the room the fight is in: a small room (≤ 30 × 30 ft) is one zone, a large space has a centre and four sides, a corridor only its passages; plus just outside and out of range. Move one zone per turn, flee two (opportunity attacks); melee in the same zone, ranged by weapon range (≤ 30 ft: the next zone), −3 against someone who attacked you in melee last round (idea of the zone combat of "Ultimate Dungeon Terrain", Dungeon Craft — reimplemented as rules, no material copied). Cover and hiding from the SRD 5.2.1 rules (half/three-quarters cover +2/+5 AC; Hide → Invisible). Explained in the Guide's own tab "Zone combat". |
| Host version | **Current Esobold** (`remoteManagement`, 2026-09-24) | The mod is not public: it targets the current Esobold from the local clone `../esobold`, may use our own Esobold PRs before they are merged, and keeps fallbacks for hosts without them. Running and updating: ARCHITECTURE §2. |
...
- Combat uses zones of the room (no grid) with moving, fleeing, cover and hiding.
