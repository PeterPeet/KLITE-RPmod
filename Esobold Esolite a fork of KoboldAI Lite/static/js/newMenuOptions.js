...
display_settings = () => {
    ...
    const corpoHideEl = document.getElementById("corpoHideLeftPanel");
    if (corpoHideEl) corpoHideEl.checked = !!localsettings.corpoHideLeftPanel;
}
...
let applyCorpoLeftpanelHidden = () => {
    ...
    } catch (_) { /* minimal impact: swallow errors */ }
}

// Ensures the hidden state reapplies after startup populates the panel probably overkill
if (typeof window !== 'undefined') {
    const originalPopulateCorpoLeftpanel = window.populate_corpo_leftpanel;
    if (typeof originalPopulateCorpoLeftpanel === 'function') {
        window.populate_corpo_leftpanel = function(...args) {
            try { originalPopulateCorpoLeftpanel.apply(this, args); } finally { applyCorpoLeftpanelHidden(); }
        }
    } else {
        window.addEventListener('load', () => {
            const orig = window.populate_corpo_leftpanel;
            if (typeof orig === 'function') {
                window.populate_corpo_leftpanel = function(...args) {
                    try { orig.apply(this, args); } finally { applyCorpoLeftpanelHidden(); }
                }
            } else {
                let tries = 0;
                const t = setInterval(() => {
                    const fn = window.populate_corpo_leftpanel;
                    if (typeof fn === 'function') {
                        clearInterval(t);
                        window.populate_corpo_leftpanel = function(...a) {
                            try { fn.apply(this, a); } finally { applyCorpoLeftpanelHidden(); }
                        }
                    }
...
confirm_settings = () => {
    ...
    const corpoHideEl = document.getElementById("corpoHideLeftPanel");
    if (corpoHideEl) localsettings.corpoHideLeftPanel = (corpoHideEl.checked ? true : false);
    updateEditorState();
    originalConfirmSettings();
    updateLegacySaveButtonState();
    applyCorpoLeftpanelHidden();
}
...
window.addEventListener('load', () => {
    ...
    settingLabelElem = createSettingElemBool(
        "corpoHideLeftPanel",
        "Hide left panel in Corpo Theme",
        "If this option is enabled the left panel in Corpo gets hidden, you can access context over the plus button"
    )
    lastSettingContainer.append(settingLabelElem)
