...
(function() {
    ...
    async function init() {
        ...
        // Check if we should show overlay or resume chat
        if (state.setupComplete) {
            log('Setup already complete, showing simplified chat');

            // Show simplified chat UI elements
            showSimplifiedChat();

            // If user was in advanced mode, switch back to it
            if (!state.easyMode) {
                switchToAdvancedMode();
            }
        } else {
            log('Showing setup overlay');
            showOverlay();
        }
